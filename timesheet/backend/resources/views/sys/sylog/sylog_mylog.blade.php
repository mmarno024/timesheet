<div class="row">
    <div class="col-sm-6">
        <div class="input-group">
            <input type="text" id="sflookup-search-key" class="form-control input-sm" ng-enter="oSearchLookup()" placeholder="Search" value="{{$request->q}}">
    ...
    <table class="table table-condensed" style="white-space: nowrap;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$v->created_at}}</td>
                <td>{{@$v->rel_created_by->username}}</td>
                <td>{{$v->activity}}</td>
                <td>{{$v->tag}}</td>
            </tr>
            <?php endforeach?>
...
{!! $data->appends(['q' => @$request->q,'limit'=>@$request->limit,'orderby'=>isset($request->orderby)?$request->orderby:'id','isactive'=>@$request->isactive,'plant'=>@$request->plant,'trs'=>@$request->trs,'doc_no'=>@$request->doc_no])->render() !!}
<script type="text/javascript">
$("#sfdialog-seelog_ #sflookup-limit").val("{{$request->limit}}");
...
function oSearchLookup(){
	SfSeeLog(SfBaseUrl + '/sys_sylog_seelog/{{$request->trs}}/{{$request->doc_no}}?q=' + $('#sfdialog-seelog_ #sflookup-search-key').val()+'&limit=' + $('#sfdialog-seelog_ #sflookup-limit').val());
}
</script>