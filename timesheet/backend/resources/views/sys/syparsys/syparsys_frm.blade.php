...
<!-- ------------------------------------------------------------------------------- -->
@section('title')System Parameter @stop
<!-- ------------------------------------------------------------------------------- -->
...
<span ng-click="f.tab='list'">Advance</span> @stop
<!-- ------------------------------------------------------------------------------- -->
@section('content')
                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body table-responsive">
                        <table class="table table-condensed table-striped table-hover">
                            ...
                            <tr class="pointer text-bold" ng-click="oDisp('{{ $v->parid }}','{{ $v->parname }}')">
                                <td>
                                    <?php if ($v->isplant == 1): ?>
                                    <span class="label label-warning pull-right">{{ Session::get('plant') }}</span>
                                    <?php endif;?>
                                    {{ $k + 1 }}. {{ $v->parname }}
                                </td>
                            </tr>
                            <tr class="pointer" ng-click="oDisp('{{ $v->parid }}','{{ $v->parname }}')">
                                <td class="p-l-20">
                                    {{ $v->isplant == 1 ? \App\Sf::getJson($v->parvalue, Session::get('plant')) : $v->parvalue }}
                                </td>
                            </tr>
                            <?php endforeach;?>
                        </table>
    ...
    <div class="panel panel-primary" ng-show="f.tab=='frm' || f.tab=='list'">
                ...
                <div id="div1" class="table-responsive">
                    <table ng-table="tableList" show-filter="false" class="table table-condensed table-hover"
                        ...
                        <tr ng-repeat="v in $data" class="pointer" ng-click="oShow(v.parid)">
                            ...
                            <td title="'Group'" filter="{pargroup: 'text'}" sortable="'pargroup'">@{{ v . pargroup }}
                            </td>
                            <td title="'Parameter Name'" filter="{parname: 'text'}" sortable="'parname'">
                                @{{ v . parname }}</td>
                            <td title="'Value'" filter="{parvalue: 'text'}" sortable="'parvalue'">@{{ v . parvalue }}
                            </td>
                            <td title="'Note'" filter="{parnote: 'text'}" sortable="'parnote'">@{{ v . parnote }}</td>
                            <td title="'Input Type'" filter="{input_type: 'text'}" sortable="'input_type'">
                                @{{ v . input_type }}</td>
                            <td title="'Specific Plant'" filter="{isplant: 'text'}" sortable="'isplant'">
                                @{{ v . isplant }}</td>
                            <td title="'Option Pattern'" filter="{option_value: 'text'}" sortable="'option_value'">
                                @{{ v . option_value }}</td>
                        </tr>
                    </table>
                </div>
            ...
            <div ng-show="f.tab=='frm'">
                <form action="#" name="frm" id="frm">
                    <div class="row">
                        <div class="col-sm-4">
                            <label>ID</label>
                            <input type="text" ng-model="h.parid" id="h_parid" class="form-control input-sm"
                                ng-readonly="f.crud!='c'" maxlength="20">
                            <label>Group</label>
                            <div class="input-group">
                                ...
                                <div class="input-group-btn">
                                    <button type="button" class="btn btn-sm btn-default" ng-click="addCombo('pargroup')"><i
                                            class="fa fa-plus"></i></button>
                                </div>
                            </div>
                            <label>Parameter Name</label>
                            <input type="text" ng-model="h.parname" id="h_parname" class="form-control input-sm" required
                                maxlength="30">
                        </div>
                        <div class="col-sm-4">
                            <label>Value</label>
                            <input type="text" ng-model="h.parvalue" id="h_parvalue" class="form-control input-sm" readonly
                                maxlength="">
                            <label>Note</label>
                            <input type="text" ng-model="h.parnote" id="h_parnote" class="form-control input-sm"
                                maxlength="30">
                            <label>Input Type</label>
                            <select ng-model="h.input_type" id="h_input_type" class="form-control input-sm" required>
                                <option
                                    ng-repeat="v in [['text','text'],['textarea','textarea'],['date','date'],['number','number'],['select','select']]"
                        ...
                        <div class="col-sm-4">
                            ...
                            <select ng-model="h.isplant" id="h_isplant" class="form-control input-sm">
                                <option ng-repeat="v in [['1','Yes'],['0','No']]" ng-value="v[0]">@{{ v[1] }}
                                </option>
                            </select>
                            <label>Option Pattern</label>
                            <textarea ng-model="h.option_value" id="h_option_value"
    ...
    <script>
        app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', function($scope, $http, NgTableParams,
            SfService) {
            SfService.setUrl("{{ url('sys_syparsys') }}");
            $scope.f = {
                crud: 'c',
                tab: 'dash',
                trash: 0,
            ...
            $scope.h = {};
            $scope.dataset = {
                pargroup: ['General', 'WEB', 'SYS-MAIL', 'SYS-API']
            };
            ...
            $scope.oDisp = function(id, sname) {
                SfDialog("disp_parsys", sname + '.', "modal-lg", function() {
                    ...
                    SfService.httpGet(SfService.getUrl('_disp'), {
                        id: id,
                        plant: $scope.f.plant
                    }, function(jdata) {
