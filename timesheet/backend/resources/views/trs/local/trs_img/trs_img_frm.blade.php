...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope,
    ...
    SfService.setUrl("{{ url('trs_local_trs_img') }}");
    $scope.f = {
        crud: 'c',
        tab: 'list',
        trash: 0,
    ...
    $scope.m = [];
    $scope.q = {
        date1: jsDate("{{ date('Y-m-d 00:00:01') }}"),
        date2: jsDate("{{ date('Y-m-d 23:59:59') }}"),
    };
    $scope.order = 'desc';
    ...
    $scope.oLookup = function(id, selector, obj) {
        switch (id) {
            case 'plant':
                ...
                break;
            case 'kd_logger':
                SfLookup("{{ url('trs_local_mst_logger_lookup') }}?plant=" + $scope.f.plant,
                    function(id, name, jsondata) {
                ...
                break;
                case 'kd_hardware':
                    SfLookup("{{ url('trs_local_mst_hardware_lookup2') }}?plant=" + $scope.f.plant + "&plantx=" + $scope.q.plantx,
                        function(id, name, jsondata) {
