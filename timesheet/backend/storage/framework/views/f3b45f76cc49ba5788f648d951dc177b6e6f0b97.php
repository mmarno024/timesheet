<!-- ------------------------------------------------------------------------------- -->
<?php $__env->startSection('title'); ?>Comment and Chat <?php $__env->stopSection(); ?>
<!-- ------------------------------------------------------------------------------- -->
            ...
            <div id="div1" class="table-responsive">
                <table ng-table="tableList" show-filter="false" class="table table-condensed table-hover" style="white-space: nowrap;">
                    <tr ng-repeat="v in $data" class="pointer" ng-click="oShow(v.id)">
                        ...
                        <td title="'Comment Category'" filter="{ctg: 'text'}" sortable="'ctg'">{{v.ctg}}</td>
                        <td title="'Document No.'" filter="{doc_no: 'text'}" sortable="'doc_no'">{{v.doc_no}}</td>
                        <td title="'Parent ID'" filter="{parent_id: 'text'}" sortable="'parent_id'">{{v.parent_id}}</td>
                        <td title="'Type'" filter="{msg_type: 'text'}" sortable="'msg_type'">{{v.msg_type}}</td>
                        <td title="'Message'" filter="{msg: 'text'}" sortable="'msg'">{{v.msg}}</td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <form action="#" name="frm" id="frm">
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        <input type="text" ng-model="h.id" id="h_id" class="form-control input-sm" required readonly  maxlength=""   ng-readonly="f.crud!='c' || true " placeholder="auto">
                        <label title='ctg'>Comment Category</label>
                        <input type="text" ng-model="h.ctg" id="h_ctg" class="form-control input-sm" required readonly  maxlength="15"  >
                    </div>
                    <div class="col-sm-4">
                        <label title='doc_no'>Document No.</label>
                        <input type="text" ng-model="h.doc_no" id="h_doc_no" class="form-control input-sm" required readonly  maxlength="30"  >
                        <label title='parent_id'>Parent ID</label>
                        <input type="text" ng-model="h.parent_id" id="h_parent_id" class="form-control input-sm"  readonly  maxlength=""  >
                    </div>
                    <div class="col-sm-4">
                        <label title='msg_type'>Type</label>
                        <input type="text" ng-model="h.msg_type" id="h_msg_type" class="form-control input-sm"  readonly  maxlength="15"  >
                        <label title='msg'>Message</label>
                        <textarea ng-model="h.msg" id="h_msg" class="form-control input-sm"   ></textarea>
                    </div>
                    <div class="col-sm-4">
                    </div>
                    <div class="col-sm-4">
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    SfService.setUrl("<?php echo e(url('sys_sychat')); ?>");
    ...
    $scope.m = [];
     ...
     var uploader = $scope.uploader = new FileUploader({
        ...
        onBeforeUploadItem: function(item) {
            ...
            item.formData = [{ id: $scope.h.id, path: 'sys_sychat', s: 'i', userid: $scope.f.userid, plant: $scope.f.plant }];
        },
    ...
    $scope.oGallery = function() {
        SfGetMediaList('sys_sychat/' + $scope.h.id, function(jdata) {
            $scope.m = jdata.files;
            $scope.$apply();
        });
    }
