<!-- ------------------------------------------------------------------------------- -->
<?php $__env->startSection('title'); ?>User Guide <?php $__env->stopSection(); ?>
<!-- ------------------------------------------------------------------------------- -->
    ...
    <div class="row">
        <div class="col-sm-4 p-0 hidden-print">
            ...
            <div class="input-group p-b-15 p-t-15">
                <input type="text" class="form-control input-sm" ng-model="f.q" ng-enter="oSearchNav()" placeholder="Search">
                <div class="input-group-btn">
                    <button type="button" class="btn btn-default btn-sm" ng-click="oSearchNav()"><i class="fa fa-search"></i></button>
                </div>
            </div>
            <table ng-table="tableNav" show-filter="false" class="table d table-hover">
                <tr ng-repeat="v in $data" class="pointer " ng-click="oRead(v.id)">
                    <td title="'Guidance List'" filter="{subj: 'text'}" sortable="'subj'" ng-class="{'bg-silver': v.id == h.id}"><i class="fa fa-arrow-right"></i> {{v.subj}}</td>
                </tr>
            </table><br><br><br>
            <hr>
                ...
                <div class="panel-body">
                    ...
                    <div>
                        ...
                        <div ng-show="m.length>0">
                            <h3>Atachment</h3>
                            <ol>
                                <li ng-repeat="v in m"><a target="_blank" href="<?php echo e(\App\Sf::fileFtpAuthUrl()); ?>/{{v.name}}">{{v.file_name}}</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
...
<div class="panel panel-success" ng-hide="f.tab=='dash'">
    ...
    <div class="panel-body">
        <div class="m-b-5 form-inline">
            ...
            <button type="button" class="btn btn-sm btn-inverse" ng-click="f.tab='dash';oSearchNav();" ng-title="Kembali ke Halaman Awal"><i class="fa fa-arrow-left"></i> Home</button>
        ...
        <div ng-show="f.tab=='frm'">
            <form action="#" name="frm" id="frm">
                <div>
                    <label>Subject</label>
                    <input type="text" ng-model="h.subj" id="h_subj" class="form-control input-sm" required maxlength="200">
                </div>
                <div class="p-t-15">
                    <textarea id="wysihtml5" ng-model="h.body" id="h_body" class="form-control input-sm" rows="15"></textarea>
                </div>
                <div class="row p-t-5">
                    <div class="col-sm-4">
                    ...
                    <div class="col-sm-4">
                        ...
                        <select ng-model="h.plant" id="h_plant" class="form-control input-sm">
                            ...
                            <option ng-value="<?php echo e($v->plant); ?>"><?php echo e($v->plant); ?> <?php echo e($v->plantname); ?></option>
                            <?php endforeach?>
                        </select>
                    </div>
                    </div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    SfService.setUrl("<?php echo e(url('sys_syguide')); ?>");
    $scope.f = { crud: 'c', tab: 'dash', trash: 0, userid: "<?php echo e(Auth::user()->userid); ?>", plant: "<?php echo e(Session::get('plant')); ?>", arrcat: [], cat: '' };
    <?php foreach ($syguide_cat as $key => $v): ?>
    $scope.f.arrcat.push(["<?php echo e($v->cat); ?>", "<?php echo e($v->cat); ?>"]);
    <?php endforeach?>
    $scope.h = {};
    $scope.m = [];
    ...
    var uploader = $scope.uploader = new FileUploader({
        ...
        onBeforeUploadItem: function(item) {
            item.formData = [{ id: $scope.h.id, path: 'sys_syguide', s: 'i', userid: $scope.f.userid, plant: $scope.f.plant }];
        },
    ...
    $scope.oRead = function(id) {
        SfService.httpGet(SfService.getUrl("_read"), { id: id }, function(jdata) {
            $("#body-show").html('');
            $scope.h = jdata.data.h;
...
<?php echo $__env->make('layouts.coloradmin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\tatonas\besai\backend\resources\views/sys/syguide/syguide_frm.blade.php ENDPATH**/ ?>