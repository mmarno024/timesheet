<!-- ------------------------------------------------------------------------------- -->
<?php $__env->startSection('title'); ?>Device <?php $__env->stopSection(); ?>
<!-- ------------------------------------------------------------------------------- -->
    ...
    <div class="panel-body">
        <div class="m-b-5 form-inline">
            <div class="pull-right">
                <div ng-show="f.tab=='list'">
                    <?php $__env->startComponent('layouts.common.coloradmin.guide',['tag'=>'trs_local_jn_device']); ?> <?php echo $__env->renderComponent(); ?>
                ...
                <div ng-show="f.tab=='frm'">
                    ...
                    <span ng-if="f.crud!='c'"> <?php $__env->startComponent('layouts.common.coloradmin.chat',['route'=>'trs_local_jn_device','id'=>'h.kd_device']); ?>  <?php echo $__env->renderComponent(); ?> </span>
                </div>
            </div>
        ...
        <div ng-show="f.tab=='list'">
            ...
            <div id="div1" class="table-responsive">
                <table ng-table="tableList" show-filter="false" class="table table-condensed table-hover" style="white-space: nowrap;">
                    <tr ng-repeat="v in $data" class="pointer" ng-click="oShow(v.kd_device)">
                        <td title="'Kd Device'" filter="{kd_device: 'text'}" sortable="'kd_device'">{{v.kd_device}}</td>
                        <td title="'Nm Device'" filter="{nm_device: 'text'}" sortable="'nm_device'">{{v.nm_device}}</td>
                        <td title="'Map To'" filter="{map_to: 'text'}" sortable="'map_to'">{{v.map_to}}</td>
                        <td title="'Channels'" filter="{channels: 'text'}" sortable="'channels'">{{v.channels}}</td>
                    </tr>
                </table>
            </div>
        </div>
        <div ng-show="f.tab=='frm'">
            <form action="#" name="frm" id="frm">
                <div class="row">
                    <div class="col-sm-4">
                        <label title='kd_device'>Kd Device</label>
                        <input type="text" ng-model="h.kd_device" id="h_kd_device" class="form-control input-sm"    maxlength=""   ng-readonly="f.crud!='c'  " >
                        <label title='nm_device'>Nm Device</label>
                        <input type="text" ng-model="h.nm_device" id="h_nm_device" class="form-control input-sm"    maxlength="30"  >
                    </div>
                    <div class="col-sm-4">
                        <label title='map_to'>Map To</label>
                        <input type="text" ng-model="h.map_to" id="h_map_to" class="form-control input-sm"    maxlength="10"  >
                        <label title='channels'>Channels</label>
                        <input type="text" ng-model="h.channels" id="h_channels" class="form-control input-sm"    maxlength=""  >
                    </div>
                </div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    SfService.setUrl("<?php echo e(url('trs_local_jn_device')); ?>");
    ...
    $scope.m = [];
     ...
     var uploader = $scope.uploader = new FileUploader({
        ...
        onBeforeUploadItem: function(item) {
            ...
            item.formData = [{ id: $scope.h.kd_device, path: 'trs_local_jn_device', s: 'i', userid: $scope.f.userid, plant: $scope.f.plant }];
        },
    ...
    $scope.oGallery = function() {
        SfGetMediaList('trs_local_jn_device/' + $scope.h.kd_device, function(jdata) {
            $scope.m = jdata.files;
            $scope.$apply();
        });
    }
    ...
    $scope.oLookup = function(id, selector, obj) {
     ...
     $scope.oLog=function(){
        SfLog('trs_local_jn_device',$scope.h.kd_device);
    }
...
<?php echo $__env->make('layouts.coloradmin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\tatonas\webmon\backend\resources\views/trs/local/jn_device/jn_device_frm.blade.php ENDPATH**/ ?>