<span ng-app="sfApp" ng-controller="guideCtrl" id="guideCtrl">
    <a href="JavaScript:Void(0);" class="btn btn-sm btn-info" title="Documentation" data-toggle="modal" data-target="#modal_guide_more1" ng-click="modalguidelistGuide()"><i class="fa fa-question-circle-o"></i>  {{counter}}</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div ng-repeat="v in modalguidelist">
                        <h3 class="text-success">{{v.subj}}</h3>
                        <div class="text-muted p-b-15">Categories : {{v.cat}} <br>Created by {{v.created_by}} at {{moment(v.created_at).format('DD MMMM YYYY')}} , updated {{moment(v.updated_at).format('DD MMMM YYYY')}}</div>
...
app.controller('guideCtrl', ['$scope', '$http', 'SfService', function($scope, $http, SfService) {
    ...
    $scope.modalguide_f = { userid: "<?php echo e(Auth::check()?Auth::user()->userid:''); ?>" };
    ...
    $scope.modalguidelistGuide = function(counter_only) {
        if ($scope.getTag() == undefined) {
            ...
            SfService.httpGet("<?php echo e(url('sys_syguide_read_bytag')); ?>", { tag: $scope.getTag(),counter_only:counter_only }, function(jdata) {
...
</div><?php /**PATH C:\xampp\htdocs\tatonas\psda\psda2\backend\resources\views/layouts/common/coloradmin/guide.blade.php ENDPATH**/ ?>