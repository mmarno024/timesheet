            ...
            <div class="panel panel-inverse">
                <div class="panel-heading">
                    <div class="panel-heading-btn">
                        <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-success" data-click="panel-reload" ng-click="oTodaystaskdata()"><i class="fa fa-repeat"></i></a>
                    </div>
                    ...
                    <div ng-repeat="v in todo">
                        ...
                        <ul>
                            <li ng-repeat="va in v.rel_d3" class="pointer p-b-10" ng-click="oActivity(v,va);">
                                <a href="javascript:void(0)">{{va.plan_start}} to {{va.plan_end}} : <i class="text-success">{{va.activity}}</i> {{va.note}}<b class="text-danger">{{va.progress}}%</b></a>
                            </li>
                        </ul>
                    </div>
                </div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    ...
    $scope.f = { crud: 'c', tab: 'list', trash: 0, userid: "<?php echo e(Auth::user()->userid); ?>", username: "<?php echo e(Auth::user()->username); ?>", plant: "<?php echo e(Session::get('plant')); ?>", cat: "<?php echo e(@$request->cat); ?>" };
    $scope.h = {};
    $scope.m = [];
    $scope.todo = [];

    $scope.oTodaystaskdata = function() {
        SfService.get(SfService.getUrl('_todaystask'), { userid: $scope.f.userid, plant: $scope.f.plant, cat: '%' }, function(jdata) {
            $scope.todo = jdata.data.data;
        });
    }

    $scope.oActivity=function(v,va){
        window.location.href="<?php echo e(url('trs_local_nprojactv')); ?>?cat="+v.cat+"&todo="+va.id;
    }
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.coloradmin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\dsn\smart\smart_back\resources\views/sys/system/sfhome.blade.php ENDPATH**/ ?>