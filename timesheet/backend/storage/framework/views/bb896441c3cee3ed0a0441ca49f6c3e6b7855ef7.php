<!-- ------------------------------------------------------------------------------- -->
<?php $__env->startSection('title'); ?>Visit <?php $__env->stopSection(); ?>
<!-- ------------------------------------------------------------------------------- -->
        ...
        <div class="m-b-5 form-inline">
            <div class="pull-right">
                <div ng-show="f.tab=='list'">
                    <?php $__env->startComponent('layouts.common.coloradmin.guide',['tag'=>'trs_local_tr_visit']); ?> <?php echo $__env->renderComponent(); ?>
                ...
                <div ng-show="f.tab=='frm'">
                    ...
                    <span ng-if="f.crud!='c'"> <?php $__env->startComponent('layouts.common.coloradmin.chat',['route'=>'trs_local_tr_visit','id'=>'h.rowid']); ?>  <?php echo $__env->renderComponent(); ?> </span>
                </div>
            </div>
            ...
            <div id="div1" class="table-responsive">
                <table ng-table="tableList" show-filter="false" class="table table-condensed table-hover" style="white-space: nowrap;">
                    <tr ng-repeat="v in $data" class="pointer" ng-click="oShow(v.rowid)">
                        ...
                        <td title="'User Id'" filter="{user_id: 'text'}" sortable="'user_id'">{{v.user_id}}</td>
                        <td title="'Url'" filter="{url: 'text'}" sortable="'url'">{{v.url}}</td>
                        <td title="'Method'" filter="{method: 'text'}" sortable="'method'">{{v.method}}</td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <form action="#" name="frm" id="frm">
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        <input type="text" ng-model="h.tanggal" id="h_tanggal" class="form-control input-sm"    maxlength=""  >
                    </div>
                    <div class="col-sm-4">
                        <label title='ip_addr'>Ip Addr</label>
                        <input type="text" ng-model="h.ip_addr" id="h_ip_addr" class="form-control input-sm"    maxlength="50"  >
                        <label title='user_id'>User Id</label>
                        <input type="text" ng-model="h.user_id" id="h_user_id" class="form-control input-sm"    maxlength="20"  >
                    </div>
                    <div class="col-sm-4">
                        <label title='url'>Url</label>
                        <input type="text" ng-model="h.url" id="h_url" class="form-control input-sm"    maxlength="65535"  >
                        <label title='method'>Method</label>
                        <input type="text" ng-model="h.method" id="h_method" class="form-control input-sm"    maxlength="10"  >
                    </div>
                </div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    SfService.setUrl("<?php echo e(url('trs_local_tr_visit')); ?>");
    ...
    $scope.m = [];
     ...
     var uploader = $scope.uploader = new FileUploader({
        ...
        onBeforeUploadItem: function(item) {
            ...
            item.formData = [{ id: $scope.h.rowid, path: 'trs_local_tr_visit', s: 'i', userid: $scope.f.userid, plant: $scope.f.plant }];
        },
    ...
    $scope.oGallery = function() {
        SfGetMediaList('trs_local_tr_visit/' + $scope.h.rowid, function(jdata) {
            $scope.m = jdata.files;
            $scope.$apply();
        });
    }
    ...
    $scope.oLookup = function(id, selector, obj) {
     ...
     $scope.oLog=function(){
        SfLog('trs_local_tr_visit',$scope.h.rowid);
    }
...
<?php echo $__env->make('layouts.coloradmin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\tatonas\webmon\backend\resources\views/trs/local/tr_visit/tr_visit_frm.blade.php ENDPATH**/ ?>