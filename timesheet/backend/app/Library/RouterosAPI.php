...
namespace App\Library {
	class RouterosAPI {
		...
		// start add by arif
		/*
	        last-seen : never =>belum pernah dipakai

*/
		public static function deleteVoucher($host, $user, $pass, $voucher_id) {
			$API = new RouterosAPI();
			// $API->debug = true;
		...
		public static function getListVoucher($host, $user, $pass) {
			$API = new RouterosAPI();
			// $API->debug = true;
			if ($API->connect($host, $user, $pass)) {
				...
				$API->write('/tool/user-manager/profile/getall');
		...
		public static function getListConnect($host, $user, $pass) {
			$API = new RouterosAPI();
			// $API->debug = true;
			if ($API->connect($host, $user, $pass)) {
				$ARR = $API->comm("/ip/hotspot/active/print
					=stats=");
		...
		public static function killUserConnection($host, $user, $pass, $no) {
			$API = new RouterosAPI();
			// $API->debug = true;
			if ($API->connect($host, $user, $pass)) {
				$ARR = $API->comm("/ip/hotspot/active/remove
				=.id=$no");
		...
		public static function createVoucher($host, $user, $pass, $prefix = '', $profile = '') {
			$API = new RouterosAPI();
			if ($API->connect($host, $user, $pass)) {
				if ($prefix) {$prefix = "9";}
				$v_user = $prefix . RouterosApi::generateRandomString(3);
				// validate($v_user, $API);
