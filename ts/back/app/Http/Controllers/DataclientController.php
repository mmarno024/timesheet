        ...
        if ($cek_user) {
            if ($cek_user->status == 'Aktif') {
                if ($cek_user->expired_date >= $today) {
                    ...
                    if ($cek_hw) {
                        $q_cek = "SELECT
                            a.`id_instansi`,b.`id_stasiun`,b.`kd_kabupaten`,f.`nm_device`
                            FROM `ms_instansi` a
                        ...
                        $raw_cek = DB::connection('wmdb')->select(DB::raw($q_cek));
                        $data_cek1 = '';
                        $data_cek2 = '';
                        foreach ($raw_cek as $kx => $vx) {
                            $data_cek1 = $vx->kd_kabupaten;
                            $data_cek2 = $vx->nm_device;
                        }
                        // return $data_cek;
                        if ($data_cek1 == 0 || $data_cek1 == NULL) {
                            if ($data_cek2 != 'GPA') {
                                $q = "SELECT
                                ...
                                e.`level`,e.`id_sensor`,e.`wkt_terima`,e.`capture`,e.`data_sampel`,e.`data_aktual`,e.`data_harian`,
                            ...
                            if ($data_cek2 != 'GPA') {
                                ...
                                d.`level_skrg`,d.`satuan`,d.`marker`,d.`tgl_update`,
                                e.`level`,e.`id_sensor`,e.`wkt_terima`,e.`capture`,e.`data_sampel`,e.`data_aktual`,e.`data_harian`,
                                f.`kd_device`,f.`nm_device`,g.`kd_propinsi`,g.`kd_kabupaten`,g.`nm_kabupaten`,h.`nm_propinsi`
                                FROM `ms_instansi` a
                                INNER JOIN `ms_stasiun` b ON b.`id_instansi`=a.`id_instansi`
                                INNER JOIN `ms_logger` c ON c.`id_stasiun`=b.`id_stasiun`
                                INNER JOIN `ms_channel` d ON d.`id_logger`=c.`id_logger`
                        ...
                        foreach ($raw as $k => $v) {
                            ...
                            if ($data_cek1 == 0 || $data_cek1 == NULL) {
                                $data['nm_kabupaten'] = 'Belum diisi';
                                $data['nm_provinsi'] = 'Belum diisi';
                            } else {
